  ...
  try {
    ...
    if (!req.files.image) {
      ...
      await image.mv(path.join(__dirname, "..", 'uploads',fileName), async (err) => {
        ...
        await UserModel.findByIdAndUpdate(newPost?.creator, {$push: { posts: newPost?._id ,profilePhoto: newPost?.profilePhoto},
        });
...
const updatePost = async (req, res, next) => {
  ...
  if(post?.creator != req.user.id){
     return next(new HttpError("You can't update this post since you are not the creator",422));
  }
  const updatePost = await PostModel.findByIdAndUpdate(postId,{body},{new:true})

  res.json({message:"Updated post details successfully",updatePost}).status(200)
  } catch (error) {
    return next(new HttpError(error));
  }
};
...
const deletePost = async (req, res, next) => {
  ...
  // check if creator of the post is the logged in user
  if(post?.creator != req.user.id){
     return next(new HttpError("You can't delete this post since you are not the creator",422));
  }
  const deletePost = await PostModel.findByIdAndDelete(postId)
await UserModel.findByIdAndUpdate(post?.creator,{$pull:{posts:post?._id}})
  res.json({message:"Delete post details successfully",deletePost}).status(200)
  } catch (error) {
    return next(new HttpError(error));
  }
};
